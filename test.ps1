...
$gpu = Get-CimInstance Win32_VideoController |
    ForEach-Object {
        # DriverDate on tihti kujul 20220315000000.000000+000
        $driverDateNice = $null
        if ($_.DriverDate) {
            try { $driverDateNice = ([Management.ManagementDateTimeConverter]::ToDateTime($_.DriverDate)).ToString("dd.MM.yyyy") }
            catch { $driverDateNice = $_.DriverDate }
        }

        [PSCustomObject]@{
            Name                    = $_.Name
            DriverVersion           = $_.DriverVersion
            DriverDate              = $driverDateNice
